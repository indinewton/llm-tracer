terraform {
  ...
  # Run: cd infrastructure && just init-prod
...
module "dynamodb" {
  ...
  enable_pitr       = true # Enabled for prod 
}
...
module "lambda" {
  ...
  cors_origins           = "*" # var.cors_origins only needed for custom web frontend
  api_keys               = var.api_keys
  api_key_required       = "true"
  default_project_key    = var.default_project_key
  rate_limit_rpm         = 100
  log_retention_days     = 30 # Longer retention for prod (cloudwatch logs)
}
...
module "monitoring" {
  ...
  monthly_budget_limit   = 10 # increase if needed
